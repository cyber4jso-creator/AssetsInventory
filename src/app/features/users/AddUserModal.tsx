...
export function AddUserModal({ open, onOpenChange }: {
  ...
  useEffect(() => {
    ...
    if (open) {
      ...
      setStatus("active"); setErrors({}); setSubmitting(false);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success(
        ...
        `${firstName.trim()} — ${ROLE_LABELS[role as Role]}`,
      ...
      toast.error("تعذرت إضافة المستخدم", message);
    ...
    <Modal open={open} onOpenChange={onOpenChange} title="إضافة مستخدم" description="أدخل بيانات المستخدم الجديد">
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-3 pt-2">
          ...
          <Btn variant="primary" disabled={submitting} onClick={() => void handleSubmit()}>

            {submitting ? "جارٍ الإضافة..." : "إضافة المستخدم"}

          </Btn>

          <Btn variant="secondary" disabled={submitting} onClick={() => onOpenChange(false)}>إلغاء</Btn>
