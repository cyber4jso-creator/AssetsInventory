...
export function AssetReportScreen({ assetId, onNavigate }: { assetId: string | null; onNavigate: NavigateFn }) {
  ...
  const lifecycleStats = [
    ...
    { label: "القسم الحالي",         value: asset.department,                 icon: Building2,      bg: "#F0EFE9", color: "#6B7280" },
  ];
    ...
    <div className="space-y-5 print:space-y-4">
      ...
      <div className="flex items-center justify-between print:hidden">
        <div className="flex items-center gap-2 text-sm text-[#6B7280]">
          ...
          <span className="text-[#2B2B2B] font-medium">تقرير الأصل</span>
        </div>
      ...
      {/* 2. Asset Information */}
      <Card>
        <h3 className="text-sm font-semibold text-[#2B2B2B] mb-5">بيانات الأصل</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-4">
          {infoFields.map(f => (
            <div key={f.label} className="flex flex-col gap-1 pb-4 border-b border-[#E5E7EB]">
      ...
      <div>
        <h3 className="text-sm font-semibold text-[#2B2B2B] mb-3">ملخص دورة حياة الأصل</h3>
          ...
          {lifecycleStats.map(s => {
            ...
            return (
              <Card key={s.label} className="flex flex-col gap-3">
                ...
                <div>
                  <p className="text-xs text-[#6B7280] leading-tight">{s.label}</p>
                  <p className="text-sm font-bold text-[#2B2B2B] mt-1 truncate">{s.value}</p>
                </div>
              </Card>
            );
          })}
      ...
      <Card p={false}>
        <div className="p-5 border-b border-[#E5E7EB]">
          <h3 className="text-sm font-semibold text-[#2B2B2B]">سجل عمليات النقل</h3>
        </div>
        {transfers.length === 0 ? (
          <p className="text-center text-sm text-[#6B7280] py-10">لا يوجد سجل نقل لهذا الأصل</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm min-w-[640px]">
              ...
              <tbody>
                {transfers.map((t, i) => (
                  <tr key={t.id} className={i < transfers.length - 1 ? "border-b border-[#F7F6F3]" : ""}>
                    <td className="px-5 py-3.5 font-mono text-xs text-[#6B7280]">{formatDateTime(t.timestamp).date}</td>
                    <td className="px-5 py-3.5 text-[#6B7280]">{t.fromDepartment ?? t.from ?? "—"}</td>
                    <td className="px-5 py-3.5 text-[#6B7280]">{t.toDepartment ?? t.to ?? "—"}</td>
                    <td className="px-5 py-3.5 text-[#2B2B2B] font-medium">{t.performedBy}</td>
                    <td className="px-5 py-3.5 text-[#6B7280] text-xs">{t.reason ?? "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <Card p={false}>
        ...
        {maintenances.length === 0 ? (
          <p className="text-center text-sm text-[#6B7280] py-10">لا يوجد سجل صيانة لهذا الأصل</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm min-w-[720px]">
              ...
              <tbody>
                {maintenances.map((m, i) => (
                  <tr key={m.id} className={i < maintenances.length - 1 ? "border-b border-[#F7F6F3]" : ""}>
                    <td className="px-5 py-3.5 font-mono text-xs text-[#6B7280]">{formatDateTime(m.timestamp).date}</td>
                    <td className="px-5 py-3.5 text-[#2B2B2B] font-medium">{m.performedBy}</td>
                    <td className="px-5 py-3.5 text-[#6B7280] text-xs max-w-xs">{m.reason ?? "—"}</td>
                    <td className="px-5 py-3.5 text-[#6B7280] text-xs max-w-xs">{m.result ?? "—"}</td>
                    <td className="px-5 py-3.5">
                      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium whitespace-nowrap"
                        style={{ background: "#EDF3EF", color: "#3D6B47" }}>
                        <span className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ background: "#4F7C5A" }} />
                        مكتملة
                      </span>
                    </td>
                  </tr>
      ...
      {/* 6. Documents */}
      <Card>
        <h3 className="text-sm font-semibold text-[#2B2B2B] mb-4">المستندات المرفقة</h3>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {DOCUMENT_PLACEHOLDERS.map(d => (
            <div key={d.label} className="border-2 border-dashed border-[#E5E7EB] rounded-xl p-4 text-center">
              <d.icon size={18} className="mx-auto text-[#9CA3AF] mb-2" />
              <p className="text-xs font-medium text-[#6B7280]">{d.label}</p>
              <p className="text-[10px] text-[#6B7280] mt-0.5">لا يوجد ملف</p>
            </div>
          ))}
        </div>
        <p className="text-center text-xs text-[#6B7280] mt-4">لا توجد مستندات مرفقة لهذا الأصل حالياً</p>
      </Card>
